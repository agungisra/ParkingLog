...
class HitungMotorFragment : Fragment() {

    private lateinit var binding: FragmentHitungMobilBinding

    private val viewModel: HitungMotorViewModel by lazy {
        val db = BiayaDb.getInstance(requireContext())
        val factory = HitungMotorViewModelFactory(db.dao)
        ViewModelProvider(this, factory)[HitungMotorViewModel::class.java]
    }
